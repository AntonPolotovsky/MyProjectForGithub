...
class Logic (var znachenie: Int = 0, var znachenie2: Int = 0) {
    inline fun usualCompare(a: Int = znachenie, b: Int = znachenie2, condition: (Int, Int) -> Boolean) {
        if (condition(a, b)) {
            println("Числа равны")
        } else if (a < b) {
    ...
    inline fun specifiedCompare(a: Int = znachenie, condition: (Int) -> Boolean) {
        if (condition(a)) {
    ...
    fun variationOfDeposit(a: Int = znachenie) {
        when {
    ...
    fun shortMathOperations(a: Int = znachenie) {
        when {
    ...
    fun smalMathProgramm(b: Int = znachenie, c: Int = znachenie2) {
        println(
    ...
    fun easyCalculator(b: Double = znachenie.toDouble(), c: Double = znachenie2.toDouble()) {
        println(
            "Введите номер операции:\n" +
                    "1. Сложение\n" +
                    "2. Вычитание\n" +
    ...
    fun timeCalculator(seconds: Int = znachenie) {
        var a = seconds
    ...
    inline fun currencyConverter(rub: Int = znachenie, condition: (Int) -> Boolean) {
        println(
            "Введите Валюту:\n" +
                    "1. Доллары\n" +
                    "2. Евро\n" +
