...
open class Arrays()  {
    lateinit var num1 : List<Int>
    lateinit var num2 : List<Int>
    //var cond : Boolean = false
    constructor(number1 : List<Int>/* = readLine()!!.toString().split(",").map { it.toInt() }*/) : this() {
        this.num1 = number1
    }
    constructor(number1 : List<Int>/* = readLine()!!.toString().split(",").map { it.toInt() }*/, number2 : List<Int> /*= readLine()!!.toString().split(",").map { it.toInt() }*/) : this(number1) {
        this.num2 = number2
    }
    // Задача 1
    fun lessFive() { //1
    ...
    inline fun lessFiveVarArg(numbers: List<Int> = num1, condition: (Int) -> Boolean) {
        for (t in numbers) {
            if (condition(t)) {
                print("$t ")
            }
    ...
    inline fun elements(numbers: List<Int> = num1, numbers2: List<Int> = num2, condition: (Int, Int) -> Boolean) { //2
        for (t in numbers) {
            for (n in numbers2) {
                if (condition(t, n)) {
                    print("$t ")
    ...
    fun sortNum(numbers: List<Int> = num1) {
        println(numbers)
...
//                readLine()!!.toString().split(",").map { it.toInt() })
    ...
    fun merge(numbers: List<Int> = num1, numbers2: List<Int> = num2) {
    ...
    fun meanings(numbers: List<Int> = num1) {
    ...
    fun evenNumbers(array: List<Int> = num1) { //11
        for (x in array) {
            if (x == 237) {
                break
            } else if (x % 2 == 0) {
    ...
    inline fun nonElements(numbers: List<Int> = num1, numbers2: List<Int> = num2, condition: (Int, Int) -> Boolean) { //12
    ...
    inline fun countSymbol(numbers: List<Int> = num1, num: Int, condition: (Int, Int) -> Boolean) { //15
        var count = 0
        for (x in numbers) {
            if (condition(x, num)) {
                count += 1
    ...
    inline fun quarter(array: List<Int> = num1, condition: (Int) -> Boolean)  //17
    {
        for (i in array) {
            if (condition(i))
                println(i)
    ...
    inline fun unicNumbers(array: List<Int> = num1, condition: (Int, Int) -> Boolean) {
